# AI부트캠프 - 루미(Lumi) · 버추얼 아이돌 AI 에이전트 — Starter Code
...
- [v] GitHub 준비
    ...
    - [v] 3강 완성 코드를 main에 업로드
      ...
      git branch -M main
      git remote add origin https://github.com/leegyeongwon/lumi-agent.git
...
- [] GCP 서버 준비 (Compute Engine)
    ...
    - [] VM 인스턴스 생성: 이름 `lumi-server` / 리전 asia-northeast3(서울) / **e2-medium** / **Ubuntu 22.04 LTS** / 네트워크 태그 `lumi-server`
        > 💸 e2-medium은 무료 등급이 아님 (결제 계정 필요, 켜둔 시간만큼 과금) → **실습 후 VM 삭제**할 것!
    - [] 방화벽 규칙: VPC 네트워크 → 방화벽 → 규칙 만들기 — **tcp:8000 허용** · 대상 태그 `lumi-server` · 소스 `0.0.0.0/0` (VM의 태그와 규칙의 대상 태그가 일치해야 적용됨!)
